...
import { deleteRoleById } from 'apiSdk/roles';
...
function CompanyViewPage() {
  ...
  const roleHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('role', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/roles/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.role?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/roles/view/${record.id}`)} key={record.id}>
                            <Td>{record.title}</Td>
                            <Td>
